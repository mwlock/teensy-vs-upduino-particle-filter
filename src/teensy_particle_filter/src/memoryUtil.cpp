...
std::tuple<int, int, int> MemoryUtil::memInfo () {
    ...
    extern char _heap_end[], _ebss[], *__brkval;
    ...
    #if ARDUINO_TEENSY41
    extern uint64_t external_psram_size;
    ...
    #if ARDUINO_TEENSY41
    uint64_t psram = _extram_start + (external_psram_size << 20) - _extram_end;
    // printf("avail PSRAM %8d b %5d kb\n", psram, psram >> 10);
    #endif
    return std::make_tuple(stack >> 10, heap >> 10, psram >> 10);
}
